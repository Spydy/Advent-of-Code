...
def path_mapper(path):
    ...
    for instruction in path:
        ...
        i += 1
        steps_taken.append(steps_taken[i - 1] + int(instruction[1:]))

    steps_taken.pop(0)
    return wire_path, steps_taken
...
with open("input.txt", "r") as input_file:
    ...
    wire1_steps_taken = wire1_path[1]
    wire1_path = wire1_path[0]

    wire2_path = input_file.readline()
    wire2_path = wire2_path.split(",")
    wire2_path = path_mapper(wire2_path)
    wire2_steps_taken = wire2_path[1]
    wire2_path = wire2_path[0]
    ...
    for line_1 in wire1_path:
        for line_2 in wire2_path:
            ...
            if intersection_point:
                wire1_steps = wire1_steps_taken[wire1_path.index(line_1)]
                wire2_steps = wire2_steps_taken[wire2_path.index(line_2)]
                reduce_wire1_steps = abs(line_1[1][0] - intersection_point[0]) + abs(line_1[1][1] - intersection_point[1])
                reduce_wire2_steps = abs(line_2[1][0] - intersection_point[0]) + abs(line_2[1][1] - intersection_point[1])
                wire1_steps = wire1_steps - reduce_wire1_steps
                wire2_steps = wire2_steps - reduce_wire2_steps
                stepcount_list.append(wire1_steps + wire2_steps)

    print(min(stepcount_list))
